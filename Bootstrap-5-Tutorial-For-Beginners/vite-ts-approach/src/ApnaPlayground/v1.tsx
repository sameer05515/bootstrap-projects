...
import { componentNames, getComponentDetails, PlaygroundBaseV1Route } from "./utils";
...
const PlaygroundHeader = ({ param, next, prev }: { param: string; next: string; prev: string }) => {
  return (
    ...
    >
      ...
      >
        <NavLink style={{ flex: 1, textAlign: "center" }} to={param ? PlaygroundBaseV1Route : "/"}>
      ...
      >
        ...
        {prev && (
          <NavLink
            to={`${PlaygroundBaseV1Route}?tester=${prev}`}
            style={{
              flex: 1,
              textAlign: "left",
              fontSize: 10,
        ...
        {next && (
          <NavLink
            to={`${PlaygroundBaseV1Route}?tester=${next}`}
            style={{
              flex: 1,
              textAlign: "right",
              fontSize: 10,
      ...
      {!param && (
        ...
        >
          {componentNames.map((name) => (
            <NavLink
              key={name}
              to={`${PlaygroundBaseV1Route}?tester=${name}`}
